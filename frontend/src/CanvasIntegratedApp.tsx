...
function CanvasIntegratedApp() {
  ...
  // Smooth real-time streaming transcription
  const startStreamingTranscription = async () => {
    ...
    // Process audio chunks every 2 seconds for smooth experience
    streamIntervalRef.current = setInterval(processAudioChunk, 2000);
  };
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        if (!currentTitle && transcript.trim().length > 10) {
          setCurrentTitle(generateTitleFromContent(transcript));
        }
      };
...
export default CanvasIntegratedApp;