...
import type { ViewMode, RecordingEntry } from './types/library';
...
function NotionLiveTranscriptApp() {
  ...
  // Load recordings count on mount
  useEffect(() => {
    loadRecordingsCount();
  }, []);
  ...
  const loadRecordingsCount = async () => {
    try {
      const stats = await libraryStorage.getLibraryStats();
      setRecordingsCount(stats.totalRecordings);
    } catch (error) {
      console.error('Failed to load recordings count:', error);
    }
  };
  ...
  const saveRecording = async () => {
    ...
    try {
      ...
      const wordCount = liveTranscript.split(/\s+/).length;
      const estimatedCost = Math.ceil(duration / 60) * 0.006; // Rough estimate

      const recordingEntry = libraryStorage.createRecordingEntry(
        liveTranscript,
        intelligentTitle || 'Untitled Recording',
        keyTopics,
        duration,
        estimatedCost,
        pdfContext || undefined,
        currentRecordingBlob || undefined
      );
      ...
      // Optionally clear the current session
      setLiveTranscript('');
      setIntelligentTitle('');
      setKeyTopics([]);
  ...
  const handleExportRecording = (recording: RecordingEntry) => {
    // Create a downloadable text file
    const content = [
      `Title: ${recording.title}`,
      `Date: ${new Date(recording.dateCreated).toLocaleString()}`,
      ...
      recording.keyTopics.length > 0 ? `Key Topics: ${recording.keyTopics.join(', ')}` : '',
      recording.tags.length > 0 ? `Tags: ${recording.tags.map(tag => `#${tag}`).join(', ')}` : '',
      '',
      'Transcript:',
    ...
    a.href = url;
    a.download = `${recording.title.replace(/[^a-zA-Z0-9]/g, '_')}.txt`;
  ...
  return (
    ...
    >
      ...
      {/* Floating Record Button - Only show in live view */}
      {currentView === 'live' && (
        <button
